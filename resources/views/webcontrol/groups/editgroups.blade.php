                        ...
                        <div class="form-group">
                            <div class="text-right stage-group">
                                ...
                                <select id="TOPIC{{ $i }}" name="TOPIC{{ $i }}" class="text-right stage-select select2 dynamic-dropdown @error('TOPIC{{ $i }}') is-invalid @enderror">
                                    ...
                                    @php
                                    $selected = '';
                                    foreach ($str_arr as $key => $stag) {
                                    if ($stag == $poststag->id && $ok) {
                                    $selected = 'selected';
                                    unset($str_arr[$key]); // Remove the selected value from $str_arr
                                    $ok = false; // Set $ok to false to prevent selecting the same value again
                                    }
                                    }
                                    @endphp

                                    <option value="{{ $poststag->id }}" {{ $selected }}>
                                        {{ $poststag->TITLE }}
                                    </option>
                                    @endforeach
                                </select>
